...
const char * const BITCOIN_CONF_FILENAME = "race.conf";
const char * const BITCOIN_PID_FILENAME = "raced.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "race" is a composite category enabling all Race-related debug output
            if(ptrCategory->count(string("race"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RaceCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RaceCore
    // Mac: ~/Library/Application Support/RaceCore
    // Unix: ~/.racecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RaceCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RaceCore";
#else
    // Unix
    return pathRet / ".racecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty race.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override race.conf
