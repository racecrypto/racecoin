...
Some notes on how to build Race Core for Windows.
...
for Ubuntu Xenial does not produce working executables for some of the Race Core applications.
...
Note that for WSL the Race Core source path MUST be somewhere in the default mount file system, for
example /usr/src/racecoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/racecrypto/racecoin.git
    sudo chmod -R a+rw racecoin
    cd racecoin
...
way. This will install to `c:\workspace\racone`, for example:
    ...
    make install DESTDIR=/mnt/c/workspace/racecoin
...
causes two of the racecoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the racecoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the racecoin source code).