...
### Send 1000 RACE to New Addresses

Send exactly 1000 RACE to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1000 RACE.

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 RACE on the remote server and defeats the purpose of a hot/cold setup.*
...
## Update race.conf on server

If you generated a new masternode private key, you will need to update the remote `race.conf` files.

Shut down the daemon and then edit the file.

```vim .racecore/race.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `race.conf` file.
...
and comparing with the official explorer at https://explorer.racecrypto.com
