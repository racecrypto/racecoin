...
*Setup instructions for a Gitian build of Race Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Race
...
Clone the git repositories for Race Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/racecrypto/racecoin
```
...
in the Race Core repository under 'Fetch and build inputs' to install sources which require
...
To build Race Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Race Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/race/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/racecrypto/racecoin
...
./bin/gbuild --commit race=${COMMIT} --url race=${URL} ../race/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit race=${COMMIT} --url race=${URL} ../race/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit race=${COMMIT} --url race=${URL} ../race/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the race git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../race/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/racecrypto/race-detached-sigs

BTCPATH=/some/root/path/race.git
SIGPATH=/some/root/path/race-detached-sigs.git

./bin/gbuild --url race=${BTCPATH},signature=${SIGPATH} ../race/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/race-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/race-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/race-osx-build.assert
```
