...
COINNAME=RACECRYPTO																										# keine Verwendung
COINSHORT=RACE																											# Verwendung lediglich für Ausgabe
COINSHORT_LOWER=race																									# Verwendung für daemon und conf - <COINSHORT_LOWER>d & <COINSHORT_LOWER>.conf
COINTAR_LINK=https://github.com/racecrypto/racecoin/releases/download/v0.12.2.3/racecore-0.12.2-linux64.tar.bz2			# Linus-binarys (Ubuntu 16.04 64bit)
COINTAR=`basename $COINTAR_LINK`																						# Verwendung zum entpacken von <COINTAR>
COINGITHUB=https://github.com/racecrypto/racecoin																		# keine Verwendung bis dato (da selbst kompilieren nicht im Script bisher)
COINGITHUB_LOCAL=racecoin																								# Verwendung als temporäres Zielverzeichnis (entpacken <COINTAR>)
SENTINELGITHUB=https://github.com/racecrypto/sentinel																	# Sentinel-Link sofern Sentinel notwendig
COINPORT=8800																											# Verwendung zur Erstellung der conf-Datei
COINRPCPORT=8801																										# Verwendung zur Erstellung der conf-Datei
COINDAEMON=raced																										# Verwendung zum Start der Wallet
COINCLI=race-cli																										# Verwendung zum stoppen der Wallet & MN-Key erzeugen & synchronisieren
COINTX=race-tx																											# keine Verwendung			
COINCORE=.racecore																										# Datenverzeichnis - wird manuell automatisch angelegt bei coinstart.
COINCONFIG=race.conf																									# konkrete configdatei

# Ermittlung der IP des VPS
IP=`LANG=C ifconfig | grep "inet " | grep -v "127\.[0-9]\+\.[0-9]\+\.[0-9]\+" | sed 's/^[ \t]*//;s/[ \t]*$//' | cut -d" " -f2 | cut -d":" -f2 | head -n1`

# Prüfen der Ubuntu-Version (muss 16.04. sein, nicht coinspezifisch, optional)
...
# VPS updaten & upgraden (nicht coinspezifisch, optional)
updateAndUpgrade() {
...
# evtl. Swap anlegen (nicht coinspezifisch, optional)
setupSwap() {
...
# Vorraussetzungen installieren (nicht coinspezifisch, für alle coins identisch, optional wenn bereits auf VPS ausgeführt oder andere Wallet dort läuft)
installRequirements() {
...
# Download und entpacken der Linux-binary's vom Github (Ubuntu 16.04.)
downloadWallet() {
...
# Installation Sentinel falls notwendig für den Coin
installSentinel() {
...
# Wallet starten (komplett von 0 - daher vorheriges aufräumen des Datenverzeichnisses)
startWallet() {
...
# Walletsynchronisierung abwarten
syncWallet() {
...
# HAUPTPROGRAMM - Aufruf der obigen Funktionen - hier startet das Script
...
# Aufruf der Funktionen oben - ggf. nicht erwünschte Teile auskommentieren
case $answer in
